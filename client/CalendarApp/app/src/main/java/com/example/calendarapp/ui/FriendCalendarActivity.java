...
// viewing a friend's calendar

public class FriendCalendarActivity extends AppCompatActivity {
    ...
    // updates the header with current dates
    private void updateHeader() {
        binding.tvMonthYear.setText((friendName != null ? friendName + "'s" : "Friend's") + " - " + monthYearFormat.format(currentWeekStart.getTime()));
        ...
        for (int i = 0; i < 7; i++) {
            final String dn = names[i];
            final int dom = temp.get(Calendar.DAY_OF_MONTH);
    ...
    // loads friend's calendar
    private void loadFriendCalendar() {
        if (friendId == -1) return;
        repository.getMyCalendar(friendId, new Repository.RepositoryCallback<List<Event>>() {
    ...
    private void drawEvents(List<Event> events) {
        ...
        int dayWidth = (screenWidth - (int)(50 * dens)) / 7;
        ...
        for (Event e : events) {
            try {
                Date dt = dateFormat.parse(e.date);
